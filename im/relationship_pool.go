...
const (
	EXPIRE_DURATION                 = 60 * 60 //60min
	ACTIVE_DURATION                 = 10 * 60
	RELATIONSHIP_POOL_STREAM_NAME   = "relationship_stream"
	RELATIONSHIP_POOL_XREAD_TIMEOUT = 60
...
func (rp *RelationshipPool) Recycle() {
	...
	rp.items.Range(f)
	for _, k := range expItems {
		rp.items.Delete(k)
	}
	count -= int64(len(expItems))
...
func (rp *RelationshipPool) load() {
	//循环直到成功
	for {
		actionID, err := rp.getActionID()
		if err != nil {
			time.Sleep(1 * time.Second)
			continue
		}
		entryID, err := rp.getLastEntryID()
		if err != nil {
			time.Sleep(1 * time.Second)
			continue
		}
		rp.last_entry_id = entryID
		rp.action_id = actionID
		rp.dirty = false
		log.Infof("relationship pool action id:%d stream last entry id:%s",
			actionID, rp.last_entry_id)
		break
	}
}
...
func (rp *RelationshipPool) RunOnce() bool {
	...
	for {
		...
		if rp.dirty && now-lastClearTs >= RELATIONSHIP_POOL_XREAD_TIMEOUT {
